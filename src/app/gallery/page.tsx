...
const programmeAlbums = [
  { slug: 'ministry-of-social-justice-empowerment', title: 'Ministry of Social Justice & Empowerment', icon: 'emblem' },
  { slug: 'synopsis', title: 'Synopsis', icon: FileText },
  { slug: 'training-programmes', title: 'Training Programmes', icon: ClipboardCheck },
  { slug: 'concern-premises', title: 'Concern Premises', icon: Building2 },
  { slug: 'awareness-programmes', title: 'Awareness Programmes', icon: Megaphone },
  { slug: 'award-recognitions', title: 'Award Recognitions', icon: Award },
];
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 md:gap-6">
          {programmeAlbums.map((album) => {
            const IconComponent = album.icon === 'emblem' ? EmblemOfIndia : album.icon;
            return (
