...
export default function Navbar() {
  return (
    <nav className="sticky top-0 z-40 w-full border-b border-t border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-sm">
      <div className="container flex h-auto min-h-16 max-w-screen-2xl flex-col items-center justify-center gap-4 py-3 md:flex-row md:justify-between md:gap-y-2">
        <div className="flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-sm font-medium md:gap-x-6">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
